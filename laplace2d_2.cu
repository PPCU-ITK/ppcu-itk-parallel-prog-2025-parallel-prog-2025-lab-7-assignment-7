...
int main(int argc, const char** argv)
{
  ...
  cudaFuncAttributes fattr{};
  if (cudaFuncGetAttributes(&fattr, jacobi_step) != cudaSuccess) {
      fprintf(stderr, "cudaFuncGetAttributes failed\n");
  }
  const int num_regs_per_thread = fattr.numRegs;
  const int static_smem_bytes   = fattr.sharedSizeBytes;

  // Host boundaries (double for accuracy)
  const double pi_d = 2.0 * asin(1.0);
#if defined(HAS_HALF)
  ...
  for (int j = 0; j < jmax+2; j++) {
      double vL = sin(pi_d * (double)j / (double)(jmax+1));
  ...
  // Mirror into Anew
  for (int i = 0; i < imax+2; i++) {
      Anew[(0)*(imax+2)+i]        = real_from_double(0.0);
      Anew[(jmax+1)*(imax+2)+i]   = real_from_double(0.0);
  }
  for (int j = 0; j < jmax+2; j++) {
      double vL = sin(pi_d * (double)j / (double)(jmax+1));
      double vR = vL * exp(-pi_d);
      Anew[(j)*(imax+2)+0]        = real_from_double(vL);
      Anew[(j)*(imax+2)+imax+1]   = real_from_double(vR);
  }
  ...
  // Device buffers
  size_t N = (size_t)(imax+2) * (jmax+2);
  REAL_T *d_A=nullptr, *d_Anew=nullptr, *d_A_init=nullptr, *d_Anew_init=nullptr;
  ERR_T  *d_err=nullptr;
  cudaMalloc(&d_A,        N * sizeof(REAL_T));
  cudaMalloc(&d_Anew,     N * sizeof(REAL_T));
  ...
  cudaMemcpy(d_A,    A,    N * sizeof(REAL_T), cudaMemcpyHostToDevice);
  cudaMemcpy(d_Anew, Anew, N * sizeof(REAL_T), cudaMemcpyHostToDevice);
  // Snapshots para restaurar antes de cada (bx,by)
  ...
  // Parámetros comunes
  const int   K   = 5;
  const double ref_err_default = 2.421354960840227e-03;
#if defined(HAS_HALF)
  const char* env_ref = std::getenv("REF_ERR_FP16");
  const double ref_err = env_ref ? atof(env_ref) : 5.0e-3;
  const double pass_threshold_pct = 1.0;
#else
  const double ref_err = ref_err_default;
  const double pass_threshold_pct = 0.001;
#endif

  auto run_once = [&](int bx_, int by_){
      if (bx_ <= 0 || by_ <= 0 || 1LL * bx_ * by_ > 1024) return;

      // Restaura estado inicial (porque hacemos swap en cada iter)
      ...
      double err_diff_d = fabs((100.0 * ((double)error / ref_err)) - 100.0);
      printf("%5d, %0.6f\n", iter, (double)error);
      printf("Total error is within %3.15E %% of the expected error\n", err_diff_d);
      if (err_diff_d < pass_threshold_pct) printf("This run is considered PASSED\n");
      else                                 printf("This test is considered FAILED\n");
      ...
      printf("SUMMARY imax=%d jmax=%d dtype=%s flops_per_pt=%.0f points=%s bx=%d by=%d time_ms=%.3f iters=%d gflops=%.6f bandwidth_GBps=%.6f avg_iter_ms=%.6f suggested_block_threads=%d num_regs=%d min_grid_size=%d static_smem_B=%d\n",
             imax, jmax, DTYPE, FLOPS_PER_POINT, POINTS_MODE,
             bx_, by_, time_ms, iter, gflops, bandwidth_GBps, avg_iter_ms,
             suggested_block_threads, num_regs_per_thread, min_grid_size, static_smem_bytes);
  ...
  // -------- ejecución --------
  if (do_sweep) {
      // Recomendación: favorecer X múltiplo de 32
      for (int bx_ : bx_list) {
          for (int by_ : by_list) {
              if (1LL * bx_ * by_ <= 1024 && bx_ > 0 && by_ > 0) {
                  run_once(bx_, by_);
              }
          }
      }
